...
CREATE PROCEDURE inserirPessoa(IN p_nome VARCHAR(255), IN p_sexo CHAR(1), IN p_data_nascimento DATE)
BEGIN
  DECLARE v_last_id INT;

  -- Seleciona o último ID gerado na tabela de Pessoas
  SELECT MAX(id) INTO v_last_id FROM Pessoas;

  -- Incrementa o último ID gerado
  SET v_last_id = v_last_id + 1;

  -- Insere um novo registro na tabela de Pessoas
  INSERT INTO Pessoas (id, nome, sexo, data_nascimento) VALUES (v_last_id, p_nome, p_sexo, p_data_nascimento);
END//
